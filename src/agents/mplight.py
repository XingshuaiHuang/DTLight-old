...
class MPLight(SharedAgent):
    ...
    The number of output actions is the number of phase pairs, which are the same for all intersections. However,
    the keys of valid_act define which phase pairs (MAXPRESSURE) or q_values (MPLight) are valid for each 
    intersection. The phase pair is actually a pair for calculating the pressure. The final action is 
    valid_acts[intersection][index], where index is the index of the max pressure or q_value.
    """
...
class FRAP(nn.Module):
    ...
    def forward(self, states):
        ...
        for i in range(batch_size):
            act_idx = acts[i]
            if act_idx >= len(self.phase_pairs):  # ***
                act_idx = -1
            elif act_idx < 0:
                act_idx = 0
