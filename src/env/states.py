...
def drq_norm(signals):
    """
    for IDQN & IPPO.
    The output observation is a dict, whose key is the id of each intersection, value is the 'obs' of each intersection.
    Dimension of 'obs' is (1, num_lanes, 5). 5 represents 5 different state variables in each lane, including
    action index, approach, total_wait, queue, and total speed.
    """
    observations = dict()
    for signal_id in signals:  # `signal_id` is the id of a signalized intersection
        signal = signals[signal_id]
        obs = []
        act_index = signal.phase
        for i, lane in enumerate(signal.lanes):
            ...
            # The first: current phase -- binary format?
            if i == act_index:
                lane_obs.append(1)
            else:
                lane_obs.append(0)
            ...
            # lane_obs.append(signal.full_observation[lane]['total_wait'] / 28)  # TODO ***
...
def mplight(signals):
    """return: {signal_id: array([phase, queue_length, queue_length, ...]), ...}. All values are int type."""
    observations = dict()
    for signal_id in signals:
        signal = signals[signal_id]
        obs = [signal.phase]
...
def wave(signals):
    """Joint queue length and approaching vehicle count as state, 
    but only # of vehicles is used in the original MA2C paper."""
    observations = dict()
    for signal_id in signals:
        signal = signals[signal_id]
        state = []
...
def fma2c(signals):
    """FMA2C has extra observations for management agents."""
