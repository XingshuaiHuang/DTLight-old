...
class TransformSamplingSubTraj:
    ...
    def __call__(self, traj):
        ...
        # Ramdonly sample a starting index for the sub-trajectory (sequence input for ODT)
        ...
        # Pad the sequences with zeros when the length of si_to_end < max_len
...
class SubTrajectory(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        # Randomly sample a trajectory with replacement
        traj = self.trajs[self.sampling_ind[index]]
        if self.transform:
            # Then randomly sample a sub-trajectory (sequence input for ODT)
...
def create_dataloader(
    ...
    # Total number of sub-trajectories you need to sample. Each training data is a sub-trajectory
    # randomly sampled from a trajectory that is also sampled with replacement.
